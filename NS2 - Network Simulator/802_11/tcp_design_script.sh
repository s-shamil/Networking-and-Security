...
if [ "$r" == "1" ]; then
	...
	totalnodes=$((10*$(($i*2))))
	ns wireless802_11.tcl 10 $(($i*2)) 10 $cbr 2   #chng
elif [ "$r" == "2" ]; then
	packet_rate=100;
	cbr=$(echo "scale=5; 1/$packet_rate" | bc)
	totalflow=$(($i*5))
	ns wireless802_11.tcl 25 4 $(($i*5)) $cbr 2   #chng
elif [ "$r" == "3" ]; then
	pow=$(($i-1))
	packet_rate=$((10**$pow));
	cbr=$(echo "scale=5; 1/$packet_rate" | bc)
	ns wireless802_11.tcl 5 5 5 $cbr 2
elif [ "$r" == "4" ]; then
	packet_rate=100;
	cbr=$(echo "scale=5; 1/$packet_rate" | bc)
	#change value of rs_thresh
	coveragedist=$(($i*150))
	ns wireless802_11.tcl 10 5 10 $cbr $i
fi

echo "SIMULATION COMPLETE. BUILDING STAT......"
#awk -f rule_th_del_enr_tcp.awk 802_11_grid_tcp_with_energy_random_traffic.tr > math_model1.out
awk -f wireless802_11.awk wireless802_11.tr > tcp_wireless.out
