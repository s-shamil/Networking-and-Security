...
#		if(node%10==3) printf("%d lololol\n", node);
		if (int(node) >= int(node_cnt)) node_cnt = node;
#		if(node%10==3) printf("%d ------\n", node_cnt);
	}
...
#		if ( strEvent == "r" ) {
		if ( strEvent == "r" && idPacket >= idLowestPacket) {
			...
			perNode_rcvd_bytes[ node ] += nBytes;
#			printf("%15.0f\n", nBytes);
...
#		printf("%d %15.5f\n", i, retransmit[i]);
	...
	for(i = 0; i<=int(node_cnt); i++) {
		#ops += perNode_rcvd_bytes[i];
		#printf("Per node thp: %d - %d\n", i, perNode_rcvd_bytes[i]);
		perNode_rcvd_bytes[i] = (perNode_rcvd_bytes[i]*8)/rTime;
		print "Node " i ": Throughput " perNode_rcvd_bytes[i] > "PerNodeThroughput.txt"
	}
#	printf( "AverageDelay-> %15.5f PacketDeliveryRatio-> %10.2f\n", rAverageDelay, rPacketDeliveryRatio ) ;
	...
	#printf( "rThroughput-> %15.2f\nrAverageDelay-> %15.5f\nnSentPackets-> %15.2f\nnReceivedPackets-> %15.2f\nnDropPackets-> %15.2f\nrPacketDeliveryRatio-> %10.2f\nrPacketDropRatio-> %10.2f\nrTime-> %10.5f\n", rThroughput, rAverageDelay, nSentPackets, nReceivedPackets, nDropPackets, rPacketDeliveryRatio, rPacketDropRatio,rTime) ;

	#printf("total_energy_consumption-> %15.5f\ntotal_energy_consumption-> %15.5f\navg_energy_per_byte-> %15.5f\navg_energy_per_packet-> %15.5f\ntotal_retransmit-> %15.0f\n", total_energy_consumption, avg_energy_per_bit, avg_energy_per_byte, avg_energy_per_packet, total_retransmit);
	printf( "%15.5f\n%15.5f\n%15.2f\n%15.2f\n%15.2f\n%10.2f\n%10.2f\n%10.5f\n", rThroughput, rAverageDelay, nSentPackets, nReceivedPackets, nDropPackets, rPacketDeliveryRatio, rPacketDropRatio,rTime) ;
	printf("%15.5f\n%15.5f\n%15.5f\n%15.5f\n%15.0f\n", total_energy_consumption, avg_energy_per_bit, avg_energy_per_byte, avg_energy_per_packet, total_retransmit);
